...
function gondrand_get_map_locations() {
    ...
    foreach ($rows as $row) {
        ...
        if ($lat === '' && $lng === '' && stripos($address, 'Renchen') !== false) {
            $lat = '48.5931';
            $lng = '7.9810';
        }
        $out[] = ['name' => $name, 'address' => $address, 'lat' => $lat, 'lng' => $lng];
    }
    // Add the requested Cameroon destination once to installations that saved
    // the first map row before the Cameroon location existed.
    if (!get_option('gondrand_map_cameroon_v229_added', false)) {
        $has_cameroon = false;
        foreach ($out as $item) {
            if (stripos($item['address'], 'Cameroun') !== false || stripos($item['address'], 'Douala') !== false || ($item['lat'] === '4.0843751' && $item['lng'] === '9.7435138')) {
                $has_cameroon = true;
                break;
            }
        }
        if (!$has_cameroon) {
            $out[] = ['name' => 'TRAVEX GLOBAL FORWARDING – CAMEROUN', 'address' => 'Douala, Cameroun', 'lat' => '4.0843751', 'lng' => '9.7435138'];
            update_option('gondrand_map_locations', $out, false);
        }
        update_option('gondrand_map_cameroon_v229_added', 1, false);
    }
    return $out;
}
...
function gondrand_admin_page() {
    ...
    ?>
    <div class="wrap">
      <h1>Travex — modifier le site <small style="font-weight:400;color:#646970">(thème 2.2.31)</small></h1>
      ...
      <form method="post" action="<?php echo esc_url(admin_url('admin.php?page=gondrand-content')); ?>">
        ...
        $q_labels = [
            'nav_quote' => 'Nom du menu (Demande de cotation)',
            'quote_title' => 'Titre du formulaire',
            'quote_intro' => 'Texte d’intro (page demande de cotation)',
        ...
        <table class="form-table" role="presentation">
          ...
          <tr><th>E-mail des demandes de cotation</th><td><input class="regular-text" type="email" name="gondrand_quote_email" value="<?php echo esc_attr(get_theme_mod('gondrand_quote_email', get_option('admin_email'))); ?>"></td></tr>
        </table>
