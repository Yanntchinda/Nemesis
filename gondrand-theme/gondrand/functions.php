 ...
 *
 * Apparence → Personnaliser → Travex : adresse e-mail des demandes de cotation.
 */
...
add_action('after_switch_theme', function () {
    ...
    if (is_string($title) && stripos($title, 'gondrand') !== false) {
        set_theme_mod('gondrand_loc_title', 'Emplacements');
    }
...
// Trigger on Customizer save AND on direct update_option() for gondrand_* options
// (slides, locations, brands, page content, page names, hidden pages, timeline).
function gondrand_maybe_bust($option_name = '') {
    if (defined('GONDRAND_BUSTING')) return;
    $n = is_string($option_name) ? $option_name : '';
    $is_gondrand = ($n !== '' && (strpos($n, 'gondrand_') === 0 || strpos($n, 'theme_mods_') === 0));
    $is_cz = did_action('customize_save_after') > 0;
    // Detect calls from gondrand admin save handlers
    $trace = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 8);
    foreach ($trace as $f) {
        $fn = isset($f['function']) ? (string)$f['function'] : '';
        $cls = isset($f['class']) ? (string)$f['class'] : '';
        if (stripos($fn, 'gondrand_save') === 0
            || stripos($fn, 'gondrand_touch_bust') !== false
            || stripos($cls, 'WP_Customize') !== false) {
            $is_gondrand = true; break;
        }
    }
    if (!$is_gondrand && !$is_cz) return;
    define('GONDRAND_BUSTING', 1);
    if (function_exists('gondrand_touch_bust')) gondrand_touch_bust();
}
add_action('customize_save_after', 'gondrand_maybe_bust', 999);
add_action('updated_option', function ($option_name) { gondrand_maybe_bust((string)$option_name); }, 999);
add_action('added_option',   function ($option_name) { gondrand_maybe_bust((string)$option_name); }, 999);
add_action('deleted_option', function ($option_name) { gondrand_maybe_bust((string)$option_name); }, 999);
...
function gondrand_purge_button() {
    ...
    echo '<span class="description">À cliquer après une modification si le téléphone n’affiche pas le changement (automatique depuis v2.2.31, bouton de secours).</span></p>';
}
...
add_action('init', function () {
    ...
    // Keep the visitor's cookie aligned with the current bust. If it doesn't match,
    // LWS/LiteSpeed has already served a stale page (because Vary:gondrand_bust
    // makes the cache key include the cookie). Updating it here ensures the NEXT
    // request bypasses any stale copy. Combined with the JS polling reload,
    // edits propagate to Android within a few seconds.
    $bust = '';
    if (function_exists('gondrand_bust')) {
        $bust = gondrand_bust();
    }
    if ($bust !== '' && !headers_sent()) {
        $cur = isset($_COOKIE['gondrand_bust']) ? (string)$_COOKIE['gondrand_bust'] : '';
        if ($cur !== $bust) {
            setcookie('gondrand_bust', $bust, time() + 86400, COOKIEPATH ?: '/',
                COOKIE_DOMAIN ?: '', is_ssl(), true);
            $_COOKIE['gondrand_bust'] = $bust;
        }
    }
}, 0);
...
function gondrand_touch_bust() {
    ...
    // time() alone can repeat when several fields are saved in the same second.
    // Use a monotonic value plus a random suffix so every save gets a new token.
    $old = (string) get_option('gondrand_bust', '0');
    $old_time = (int) preg_replace('/[^0-9].*$/', '', $old);
    $new_bust = (string) max(time(), $old_time + 1) . '-' . wp_rand(100000, 999999);

    // Prevent updated_option from recursively triggering another purge.
    if (!defined('GONDRAND_BUSTING')) define('GONDRAND_BUSTING', 1);
    update_option('gondrand_bust', $new_bust, false);
    // Drop a cookie so LWS/LiteSpeed/CDN receives the current version token.
    if (!headers_sent()) {
        setcookie('gondrand_bust', $new_bust, time() + 86400, COOKIEPATH ?: '/',
            COOKIE_DOMAIN ?: '', is_ssl(), true);
    }
    gondrand_purge_caches();
    gondrand_write_htaccess();
    $running = false;
}
...
Header always set X-LiteSpeed-Cache-Control "no-cache, no-store, max-age=0, no-vary"
...
function gondrand_departure_items() {
    $raw = function_exists('gondrand_mod') ? gondrand_mod('gondrand_departures') : '';
    if ($raw === '') {
        return [];
    }
    $items = [];
...
function gondrand_departures_html() {
    $title = function_exists('gondrand_text')
        ? gondrand_text('gondrand_departures_title', 'Départs')
        : 'Départs';
    $items = gondrand_departure_items();
    $moving = count($items) > 1;
    if (!$items) {
        $items = ['Aucun départ publié pour le moment.'];
    }
    $visible_items = $moving ? array_merge($items, $items) : $items;
    $html = '<section class="departures-ticker" id="departures" aria-label="' . esc_attr($title) . '">';
    $html .= '<div class="wrap departures-ticker-inner">';
    $html .= '<strong class="departures-label">' . esc_html($title) . '</strong>';
    $html .= '<div class="departures-window"><div class="departures-track' . ($moving ? ' is-moving' : '') . '">';
    foreach ($visible_items as $item) {
        $html .= '<span class="departure-item">' . esc_html($item) . '</span>';
    }
    $html .= '</div></div></div></section>';
    return $html;
}

function gondrand_map_section_html() {
    $locations = function_exists('gondrand_get_map_locations') ? gondrand_get_map_locations() : [];
    $title = function_exists('gondrand_text') ? gondrand_text('gondrand_map_title', 'Nos implantations') : 'Nos implantations';
    $my_maps = function_exists('gondrand_mod') ? gondrand_mod('gondrand_map_my_maps_url') : '';
    $api_key = function_exists('gondrand_mod') ? gondrand_mod('gondrand_map_api_key') : '';

    $html = '<section class="gondrand-map-section section alt" id="carte-implantations">';
    $html .= '<div class="wrap"><div class="bar"></div><h2>' . esc_html($title) . '</h2>';
    $html .= '<p class="lead">Retrouvez nos entreprises et agences sur la carte.</p>';

    if (!$locations) {
        $html .= '<div class="gondrand-map-empty">Ajoutez une entreprise dans l’administration pour afficher sa localisation.</div></div></section>';
        return $html;
    }

    $html .= '<div class="gondrand-map-layout">';
    if ($my_maps !== '' && filter_var($my_maps, FILTER_VALIDATE_URL)) {
        $html .= '<div class="gondrand-google-map"><iframe src="' . esc_url($my_maps) . '" title="' . esc_attr($title) . '" loading="lazy" referrerpolicy="no-referrer-when-downgrade" allowfullscreen></iframe></div>';
    } elseif ($api_key !== '') {
        $html .= '<div class="gondrand-google-map" id="gondrand-google-map" role="application" aria-label="' . esc_attr($title) . '"></div>';
        $json = wp_json_encode($locations);
        $html .= '<script>window.GONDRAND_MAP_LOCATIONS=' . $json . ';window.gondrandInitMap=function(){var el=document.getElementById("gondrand-google-map");if(!el||!window.google||!google.maps)return;var rows=window.GONDRAND_MAP_LOCATIONS||[],fallback={lat:48.58,lng:7.98},center=fallback;var first=rows.find(function(x){return parseFloat(x.lat)&&parseFloat(x.lng);});if(first)center={lat:parseFloat(first.lat),lng:parseFloat(first.lng)};var map=new google.maps.Map(el,{center:center,zoom:6,mapTypeControl:false,streetViewControl:false,fullscreenControl:true});var bounds=new google.maps.LatLngBounds();var info=new google.maps.InfoWindow();var add=function(row,position){var marker=new google.maps.Marker({map:map,position:position,title:row.name||row.address||"Entreprise"});bounds.extend(position);marker.addListener("click",function(){var box=document.createElement("div"),strong=document.createElement("strong"),br=document.createElement("br"),text=document.createTextNode(row.address||"");strong.textContent=row.name||"";box.appendChild(strong);box.appendChild(br);box.appendChild(text);info.setContent(box);info.open({map:map,anchor:marker});});};var geocoder=new google.maps.Geocoder();rows.forEach(function(row){var lat=parseFloat(row.lat),lng=parseFloat(row.lng);if(lat&&lng){add(row,{lat:lat,lng:lng});}else if(row.address){geocoder.geocode({address:row.address},function(results,status){if(status==="OK"&&results[0]){add(row,results[0].geometry.location);if(rows.length>1)map.fitBounds(bounds);}});}});if(rows.length>1)setTimeout(function(){if(!bounds.isEmpty())map.fitBounds(bounds);},1200);};</script>';
        $html .= '<script async defer src="https://maps.googleapis.com/maps/api/js?key=' . rawurlencode($api_key) . '&callback=gondrandInitMap&loading=async"></script>';
    } else {
        // No Google API key: use a keyless OpenStreetMap/Leaflet map so the
        // interactive map still renders reliably on Android. Route buttons
        // below always open the exact destination in Google Maps.
        $html .= '<div class="gondrand-google-map" id="gondrand-open-map" role="application" aria-label="' . esc_attr($title) . '"></div>';
        $json = wp_json_encode($locations);
        $html .= '<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css">';
        $html .= '<script>(function(){window.GONDRAND_MAP_LOCATIONS=' . $json . ';function startMap(){var el=document.getElementById("gondrand-open-map");if(!el||!window.L)return;var rows=window.GONDRAND_MAP_LOCATIONS||[],points=rows.map(function(r){var lat=parseFloat(r.lat),lng=parseFloat(r.lng);return lat&&lng?{row:r,lat:lat,lng:lng}:null;}).filter(Boolean),fallback={lat:48.5931,lng:7.9810};if(!points.length)points=[{row:rows[0]||{name:"Travex"},lat:fallback.lat,lng:fallback.lng}];var map=L.map(el,{scrollWheelZoom:false}).setView([points[0].lat,points[0].lng],6);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:"© OpenStreetMap contributors"}).addTo(map);var bounds=L.latLngBounds([]);points.forEach(function(p){var marker=L.marker([p.lat,p.lng]).addTo(map);var box=document.createElement("div"),strong=document.createElement("strong"),br=document.createElement("br"),text=document.createTextNode(p.row.address||"");strong.textContent=p.row.name||"Entreprise";box.appendChild(strong);box.appendChild(br);box.appendChild(text);marker.bindPopup(box);bounds.extend([p.lat,p.lng]);});if(points.length>1)map.fitBounds(bounds.pad(.25));setTimeout(function(){map.invalidateSize();},250);}if(window.L){startMap();}else{var s=document.createElement("script");s.src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js";s.onload=startMap;document.head.appendChild(s);}})();</script>';
    }

    $html .= '<div class="gondrand-map-locations">';
    foreach ($locations as $row) {
        $query = trim(($row['name'] ?? '') . ' ' . ($row['address'] ?? ''));
        $destination = (!empty($row['lat']) && !empty($row['lng'])) ? ((string) $row['lat'] . ',' . (string) $row['lng']) : $query;
        $link = 'https://www.google.com/maps/dir/?api=1&destination=' . rawurlencode($destination) . '&travelmode=driving';
        $html .= '<article class="gondrand-map-location"><h3>' . esc_html($row['name']) . '</h3><p>' . nl2br(esc_html($row['address'])) . '</p><a href="' . esc_url($link) . '" target="_blank" rel="noopener">Tracer l’itinéraire dans Google Maps →</a></article>';
    }
    $html .= '</div></div></div></section>';
    return $html;
}
...
function gondrand_try_serve() {
    ...
    if ($path === '/travex-bust.json' || $path === '/travex-bust.json/') {
        ...
        header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0, post-check=0, pre-check=0, proxy-revalidate');
    ...
    if ($ext === 'html') {
        header('X-Gondrand-Theme: 2.2.31');
        header('X-LiteSpeed-Cache-Control: no-cache, no-store, max-age=0, esi=on, no-vary');
        header('X-LSCACHE: no-cache');
        header('X-LiteSpeed-Tag: ');
        header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0, post-check=0, pre-check=0, private, proxy-revalidate');
        header('Pragma: no-cache');
        header('Expires: Thu, 01 Jan 1970 00:00:00 GMT');
        header('CDN-Cache-Control: no-store');
        header('Cloudflare-CDN-Cache-Control: no-store');
...
function gondrand_quote_defaults() {
    return [
        'quote_title' => 'Demander une cotation gratuite',
...
function gondrand_cms_payload() {
    ...
    if (function_exists('gondrand_catalog')) {
        ...
        $cms['nav_quote'] = gondrand_t('nav_quote', 'Demande de cotation');
...
function gondrand_slide_rules($fit, $pos, $h, $ov, $cover_fallback, $clip_height = true) {
    ...
    if ($h && $clip_height) {
        ...
        $css .= '.slides{position:relative !important;width:100% !important;inset:auto !important;height:auto !important;min-height:1px !important;}';
        $css .= '.slide{position:absolute !important;left:0 !important;right:0 !important;top:0 !important;opacity:0 !important;visibility:hidden !important;z-index:1 !important;height:auto !important;pointer-events:none !important;-webkit-backface-visibility:hidden !important;backface-visibility:hidden !important;transition:opacity 800ms ease !important;}';
        $css .= '.slide.active{position:relative !important;opacity:1 !important;visibility:visible !important;z-index:2 !important;pointer-events:auto !important;}';
        $css .= '.slide-img{position:relative !important;inset:auto !important;width:100% !important;height:auto !important;max-width:100% !important;max-height:none !important;transform:translateZ(0) !important;-webkit-transform:translateZ(0) !important;object-fit:contain !important;object-position:' . $pos . ' !important;display:block !important;-webkit-backface-visibility:hidden !important;backface-visibility:hidden !important;}';
        $css .= '.hero-nav button{top:38% !important;}';
    }
...
.departures-ticker { background: #062544 !important; color: #fff !important; border-top: 3px solid #c9a84c !important; overflow: hidden !important; }
.departures-ticker-inner { display: flex !important; align-items: stretch !important; min-height: 52px !important; width: min(1180px, calc(100% - 40px)) !important; }
.departures-label { flex: 0 0 auto !important; display: flex !important; align-items: center !important; padding: 0 24px 0 0 !important; color: #d4b45a !important; letter-spacing: .12em !important; text-transform: uppercase !important; font-size: 13px !important; }
.departures-window { flex: 1 1 auto !important; min-width: 0 !important; overflow: hidden !important; display: flex !important; align-items: center !important; }
.departures-track { display: flex !important; align-items: center !important; width: max-content !important; min-width: 100% !important; }
.departures-track.is-moving { animation: gondrand-departures-scroll 28s linear infinite !important; }
.departures-track:hover, .departures-track:focus-within { animation-play-state: paused !important; }
.departure-item { flex: 0 0 auto !important; display: inline-flex !important; align-items: center !important; padding: 0 42px 0 0 !important; color: #fff !important; font-size: 14px !important; white-space: nowrap !important; }
.departure-item::after { content: "•" !important; color: #c9a84c !important; margin-left: 42px !important; }
@keyframes gondrand-departures-scroll { from { transform: translateX(0); } to { transform: translateX(-50%); } }
.gondrand-map-section { padding-top: 46px !important; padding-bottom: 46px !important; }
.gondrand-map-layout { display: block !important; width: 100% !important; }
.gondrand-google-map { width: 100% !important; min-height: 480px !important; background: #dbe6ef !important; overflow: hidden !important; border: 1px solid #d6dee8 !important; box-shadow: 0 8px 24px rgba(10,30,55,.10) !important; }
.gondrand-google-map iframe { width: 100% !important; height: 480px !important; border: 0 !important; display: block !important; }
#gondrand-google-map, #gondrand-open-map { min-height: 480px !important; height: 480px !important; }
.gondrand-map-locations { display: grid !important; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)) !important; gap: 14px !important; align-content: start !important; max-height: none !important; overflow: visible !important; margin-top: 18px !important; }
.gondrand-map-location { background: #fff !important; border: 1px solid #dce3eb !important; padding: 13px 15px !important; }
.gondrand-map-location h3 { color: #0a3a6b !important; font-size: 14px !important; margin: 0 0 5px !important; }
.gondrand-map-location p { color: #5c6773 !important; font-size: 13px !important; margin: 0 0 6px !important; }
.gondrand-map-location a { color: #0d4a86 !important; font-size: 12px !important; font-weight: 700 !important; }
.gondrand-map-empty { background: #fff !important; border: 1px dashed #cbd5e1 !important; padding: 24px !important; color: #5c6773 !important; }
CSS;
}
...
@media screen and (max-width: 900px) {
  ...
  .departures-ticker-inner { display: block !important; width: calc(100% - 20px) !important; padding: 9px 0 !important; }
  .departures-label { display: block !important; padding: 0 0 5px !important; font-size: 11px !important; }
  .departure-item { font-size: 13px !important; }
  .gondrand-map-layout { display: block !important; }
  .gondrand-google-map, .gondrand-google-map iframe, #gondrand-google-map, #gondrand-open-map { min-height: 300px !important; height: 300px !important; max-height: 300px !important; }
  .gondrand-map-locations { max-height: none !important; margin-top: 14px !important; }
}
CSS;
}
...
function gondrand_handle_mail() {
    ...
    $body  = "Nouvelle demande de cotation — Travex Global Forwarding\n";
    ...
    $subject = 'Demande de cotation Travex Global Forwarding';
