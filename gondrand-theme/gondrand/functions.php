...
function gondrand_purge_button() {
    ...
    echo '<span class="description">À cliquer après une modification si le téléphone n’affiche pas le changement (automatique depuis v2.2.29, bouton de secours).</span></p>';
}
...
function gondrand_map_section_html() {
    ...
    if ($my_maps !== '' && filter_var($my_maps, FILTER_VALIDATE_URL)) {
        ...
        $first = $locations[0];
        $query = trim(($first['name'] ?? '') . ' ' . ($first['address'] ?? ''));
        $src = 'https://www.google.com/maps?q=' . rawurlencode($query) . '&output=embed';
        $html .= '<div class="gondrand-google-map"><iframe src="' . esc_url($src) . '" title="' . esc_attr($title) . '" loading="lazy" referrerpolicy="no-referrer-when-downgrade" allowfullscreen></iframe></div>';
    }
...
function gondrand_try_serve() {
    ...
    if ($ext === 'html') {
        header('X-Gondrand-Theme: 2.2.29');
...
#gondrand-google-map { min-height: 480px !important; }
...
@media screen and (max-width: 900px) {
  ...
  .gondrand-google-map, .gondrand-google-map iframe, #gondrand-google-map { min-height: 300px !important; height: 300px !important; max-height: 300px !important; }
