...
function gondrand_purge_button() {
    ...
    echo '<span class="description">À cliquer après une modification si le téléphone n’affiche pas le changement (automatique depuis v2.2.28, bouton de secours).</span></p>';
}
...
function gondrand_map_section_html() {
    ...
    foreach ($locations as $row) {
        ...
        $html .= '<article class="gondrand-map-location"><h3>' . esc_html($row['name']) . '</h3><p>' . nl2br(esc_html($row['address'])) . '</p><a href="' . esc_url($link) . '" target="_blank" rel="noopener">Ouvrir dans Google Maps →</a></article>';
    }
    $html .= '</div></div></div></section>';
    return $html;
}
...
function gondrand_try_serve() {
    ...
    if ($ext === 'html') {
        header('X-Gondrand-Theme: 2.2.28');
...
.gondrand-map-layout { display: grid !important; grid-template-columns: minmax(0, 1.6fr) minmax(280px, .8fr) !important; gap: 22px !important; align-items: stretch !important; }
.gondrand-google-map { min-height: 390px !important; background: #dbe6ef !important; overflow: hidden !important; border: 1px solid #d6dee8 !important; box-shadow: 0 8px 24px rgba(10,30,55,.10) !important; }
.gondrand-google-map iframe { width: 100% !important; height: 390px !important; border: 0 !important; display: block !important; }
#gondrand-google-map { min-height: 390px !important; }
.gondrand-map-locations { display: grid !important; grid-template-columns: 1fr !important; gap: 10px !important; align-content: start !important; max-height: 390px !important; overflow: auto !important; }
